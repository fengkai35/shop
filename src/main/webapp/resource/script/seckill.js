...
var seckill= {
    ...
    URL: {
        ...
        order :function (seckillId,md5) {
            return '/shop/seckill/'+seckillId+'/'+md5+'/order';
        },
        addUser :function (userPhone) {
            return '/shop/seckill/'+userPhone+'/addUser';
        }
    },
        ...
        $.post(seckill.URL.exposer(seckillId), params, function (result) {
            ...
            if (result && result['success']){
                ...
                if (exposer['exposed']){
                    ...
                    //获取秒杀地址
                    var md5 = exposer['md5'];
                    var killUrl = seckill.URL.order(seckillId,md5);
...
//                        window.location.replace location.href=killUrl;
//                        var params2 = {};
//                        params2.orderId =seckillId;

                        window.location.replace(killUrl);
...
//                                document.write('<p>'+state+'</p>');
//                                document.write('<button class="btn btn-pay btn-lg" id="payBt">支付</button>');
//                            }
//                        });
                    });
                    node.show();
            ...
            if (!seckill.validatePhone(killPhone)) {
                ...
                $('#killPhoneBtn').click(function () {
                    ...
                    if (seckill.validatePhone(inputPhone)) {
                        //电话写入数据库
                        $.post('/shop/seckill/'+inputPhone+'/addUser');

                        //电话写入cookie
                        $.cookie('killPhone', inputPhone, {expires: 1, path: '/shop'});
                        //刷新界面
